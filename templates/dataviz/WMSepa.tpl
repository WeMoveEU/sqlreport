...
<div id="amount-by-month" class="col-md-6">
...
function drawMonthTable(dom) {
//var dim     = ndx.dimension(function(d) { return d3.timeMonth(d.dd); });
  var dateFormat = d3.timeFormat("%Y-%m");
var dim     = ndx.dimension(function(d) { return dateFormat(d.dd); });


var grp   = dim.group();
grp.bottom = grp.top;
grp.all = function() {return grp.top(Infinity)};
...
return d.type == 'OOFF'; })
     .count(true)
     .sum(function(d){console.log(d);return d.total})
/*     .value("rcur")
      .filter(function(d) { return d.type == 'RCUR'; })
     .sum(function(d){return +d.qty*+d.amount})
...
*/
  ...
  var graph=dc.dataTable(dom)
    ...
    .columns([function(d){return d.key},
              function(d){ return d.value.ooff.sum},
  //            function(d){ return d.value.rcur.sum},
...
*/
  ...
  graphs.tablemonth=drawMonthTable('#monthly-table'); 
