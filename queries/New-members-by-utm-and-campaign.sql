...
select 
campaign_name, campaign_id as speakout_id, language, sum(completed_new_member) as total_new,
...
sum(if((utm_media="email" or utm_media="mail") and utm_source != "mail_share" and utm_source !="KB" and utm_source not like "campact%",completed_new_member,0)) as forwarding_link_from_mailing,
sum(if((utm_media="email" or utm_media="mail") and (utm_source ="KB" or utm_source like "campact%"),completed_new_member,0)) as campact_seed,
...
sum(if((utm_media="email" or utm_media="mail") and utm_source != "mail_share" and utm_source !="KB" and utm_source not like "campact%",completed_new_member,0)) -
sum(if((utm_media="email" or utm_media="mail") and (utm_source ="KB" or utm_source like "campact%"),completed_new_member,0)) -
...
from
(select
  c.name as campaign_name, convert(c.external_identifier, signed ) as campaign_id, custom.language_4 as language,
