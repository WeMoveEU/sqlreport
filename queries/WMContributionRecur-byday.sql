SELECT
  cr.day,
  IFNULL(nr.new_recur, 0) - IFNULL(lr.cancelled_recur, 0) as net_recurring_donors,
  IFNULL(nr.new_amount, 0) - IFNULL(lr.cancelled_amount, 0) as net_recurring_amount,
...
FROM
  (
    SELECT DISTINCT (DATE_FORMAT(create_date, '%Y-%m-%d')) AS day
     FROM civicrm_contribution_recur
  UNION 
    SELECT DISTINCT (DATE_FORMAT(cancel_date, '%Y-%m-%d')) AS day
     FROM civicrm_contribution_recur WHERE cancel_date IS NOT NULL
  ) cr
  LEFT JOIN 
  (
    SELECT
      (DATE_FORMAT(create_date, '%Y-%m-%d')) AS day, COUNT(*) AS new_recur, SUM(amount) new_amount
    FROM (
      SELECT
        R.id,
        create_date,
  ...
  (
    SELECT
      DATE_FORMAT(cancel_date, '%Y-%m-%d') AS day, COUNT(*) AS cancelled_recur, SUM(amount) cancelled_amount, 
  ...
  (
    ...
    FROM (
      SELECT
        DATE_FORMAT(date, '%Y-%m-%d') AS day, language, country, COUNT(*) AS new_recur, SUM(amount) AS money
      FROM (
        SELECT DISTINCTROW
          R.id,
          create_date AS date,
      ...
      GROUP BY day, language, country
    ) AS recurtab2
    GROUP BY day
  ) nrl ON nrl.day = cr.day
  LEFT JOIN 
  (
    SELECT
      day,
    ...
    FROM (
      SELECT
        DATE_FORMAT(cancel_date, '%Y-%m-%d') AS day, language, country, COUNT(*) AS new_recur, SUM(amount) AS money
      FROM (
        SELECT DISTINCTROW
          R.id,
          R.cancel_date,
      ...
      GROUP BY day, language, country) AS recurtab2
    GROUP BY day
  ) lrl ON lrl.day = cr.day
