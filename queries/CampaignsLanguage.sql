select 
  ...
  (select sum(npeople) from speakeasy_petition_metrics m where activity="oneoff_count" and m.campaign_id=c.id group by c.id) oneoff_donation,
  (select sum(npeople) from speakeasy_petition_metrics m where activity="oneoff_amount" and m.campaign_id=c.id group by c.id) oneoff_donation_amount,
  (select sum(npeople) from speakeasy_petition_metrics m where activity="recurring_count" and m.campaign_id=c.id group by c.id) recurring_donation,
  (select sum(npeople) from speakeasy_petition_metrics m where activity="recurring_amount" and m.campaign_id=c.id group by c.id) recurring_donation_amount,
