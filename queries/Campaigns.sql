select 
  ...
  (select sum(npeople) from speakeasy_petition_metrics m where m.activity="unique_recipient" and m.parent_id=c.id group by parent_id) unique_recipient,
  ...
  (select sum(npeople) from speakeasy_petition_metrics m where m.activity="Petition" and status='9' and m.parent_id=c.id group by parent_id) new,
